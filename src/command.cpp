...
private
    ...
    static void CheckDnsForNic(NetworkInterface nic, List <ValidDNS> listValidDns) {
        if (!GlobalData.ReadOnly) {
            ...
            Console.WriteLine();
            Console.WriteLine(
                    "Information:\tProgram started in UPDATE mode. Optimal DNS Client configuration will be applied.");
        ...
        var
        optList = OptimalDnsListNoLoadBalancing(listValidDns, out
        sValidDnsListRemediate, nicIpAddress);
        ...
        for (var i = 0; i <= 2; i++) {
            ...
            if (optList.Count < i + 1) {
                ...
                if (isOptimalDns) {
                    Console.ResetColor();
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine("Information:\t{0} DNS Server with IPv4 address [{1}] is configured optimally.",
                                      UppercaseFirst(dnsPositionName), validDns.IpAddress);

                    Logger.Warn("{0} DNS Server with IPv4 address [{1}] is configured optimally.",
                                UppercaseFirst(dnsPositionName), validDns.IpAddress);
                } else {
                    var displayTitle = string.Format(
                            "Warning:\t{1} DNS Server with IPv4 address [{0}] is not configured optimally.",
                            validDns.IpAddress, UppercaseFirst(dnsPositionName));
                ...
                optList)
                {
                    ...
                    validDnsArray)
                    {
                        if (validDns.IpAddress.Equals(serverIp, StringComparison.CurrentCulture)) {
                            Console.WriteLine("Information:\tDNS Server {0} in domain {3} with {1} found in site {2}",
                                              validDns.ServerName, validDns.IpAddress, validDns.SiteName,
                                              validDns.DomainName);

                            Logger.Info("DNS Server {0} in domain {3} with {1} found in site {2}", validDns.ServerName,
                                        validDns.IpAddress, validDns.SiteName, validDns.DomainName);
...
private
    ...
    static void LogNicConfiguration(NetworkInterface nic) {
        var properties = nic.GetIPProperties();
...
private
    ...
    static List <string> GetRegistryValue(string registryPath, string key) {
...
public
    ...
    static List <string> GetWinsOnNicFromRegistry(string nicGuid) {
        var winsList = new List<string>();
        try {
            var key = "NameServerList";
            string
            regpath = string.Format(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\NetBT\Parameters\Interfaces\Tcpip_{0}", nicGuid);
...
public
    ...
    static bool CheckValidityOfValidDNSReduced(ref ValidDNS server) {
        var result = false;
...
public
    ...
    static bool PortCheckTcp(string iPAddress, PortCheck portCheck) {
        try {
            //tcp port parameters
            var timeOut = 200;
...
public
    ...
    static bool
    PortQueryTcpConnect(string iPAddress, int port, int timeOut, int numberOfChecksInTry, int numberOfTries) {
...
private
    ...
    static bool NotifyAndRemediateDNS(NetworkInterface nic, string dnsIpPAddress) {
        return DnsRemediate.SetDns(nic.Id, dnsIpPAddress);
    }

public

    static List<ValidDNS[]>
    OptimalDnsListNoLoadBalancing(List <ValidDNS> listValidDns, out string sOptimalDnsListNoLoadBalancingRemediate,
                                  string nicIpAddress) {
        ...
        if (listValidDns.Count > 0) {
            ...
            groupByRatingMarkList)
            {
                ...
                group)
                {
                    if (adSiteServer.IpAddress.Equals(LoopbackIPv4Address,
...
public
    ...
    static bool
    IsDnsOptimalNoLoadBalancing(string dnsIpAddress, int order, List<ValidDNS[]> optimalDnsListNoLoadBalancing) {
        try {
            ...
            server
            in
            optimalDnsListNoLoadBalancing[order])
            {

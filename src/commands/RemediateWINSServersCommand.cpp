...
{
    public class RemediateWINSServersCommand : BaseCommand
    {
        private readonly NetworkInterfaceProvider _NetworkInterfaceProvider;
        private readonly WINSServerProvider _WINSServerProvider;

        public RemediateWINSServersCommand(Options options, CommandContext context) : base(options, context)
        {
        ...
        public bool Execute()
        {
            try
            {
                ...
                Logger.Debug($"Maximum number of WINS servers to discover is {maxServertoFind}");
                ...
                Logger.Debug($"Maximum number of WINS servers to discover after discovery is {maxServertoFind}");

                bool writeDiscoveredServers = false;

                if (Options.Force)
                {
                    WriteInformation("Program is started with force option; current WINS server configuration will be overriden");
                ...
                else
                {
                    if (maxServertoFind == currentWinsServers.Count)
                    {
                        for (int i = 1; i <= maxServertoFind; i++)
                        {
                            ...
                            if (winsServer.IPAddress.Equals(currentWinsServer.ToString(), StringComparison.InvariantCultureIgnoreCase))
                            {
                                if (string.IsNullOrEmpty(winsIp))
                                {
                                    //Auto
                                    WriteSuccess($"Currently configured {serverPosition} WINS server {currentWinsServer} is configured optimaly");
                                }
                                else
                                {
                                    ...
                                    {
                                        WriteSuccess($"Manualy specified {serverPosition} WINS server {winsIp} is configured optimaly");
                                    }
                                    else
                                    {
                                        writeDiscoveredServers = true;
                                        WriteError($"Manualy specified {serverPosition} WINS server {winsIp} is not configured optimaly");
                                        WriteError($"Regardles of this error we will still configure your provided {serverPosition} WINS server");
                                    }
                                }
                            }
                            else
                            {
                                writeDiscoveredServers = true;
                                WriteError($"Currently configured {serverPosition} WINS server {currentWinsServer} is not configured optimaly");
                            }
                        }
                    }
                    else
                    {
                        WriteInformation("Currently configured WINS servers are not optimal becouse the total number of servers is not equal with the expected optimal configuration");
                        writeDiscoveredServers = true;
                    }
                }
                ...
                if (writeDiscoveredServers)
                {
                    ...
                    if (Options.ReadOnlyMode)
                    {
                        ...
                        foreach (var dnsServer in winsServerList)
                        {
                            WriteSuccess($"{GetServerName(dnsServer.Key, true)} WINS Server with {dnsServer.Value.IPAddress}");
                        }
                    ...
                    {
                        Console.WriteLine();
                        WriteSuccess($"Following WINS servers will be commited to network adapter '{Context.NetworkInterface.Name}'");
                        ...
                        bool configurationCommited = _NetworkInterfaceProvider.SetWINSServers(Context.NetworkInterface.Id, wins1, wins2);
        ...
        private Dictionary<int, IPAddress> GetCurrentWINSServers()
        {
        ...
        private void LogCurrentWINSServer(Dictionary<int, IPAddress> currentDnsServers)
        {
            foreach (var dnsServer in currentDnsServers)
            {
                Logger.Debug($"Discoverd WINS{dnsServer.Key} server IP {dnsServer.Value}");
            }
        }

        private Dictionary<int, WindowsServer> SetDiscoveredWINSServers(IList<WindowsServer> discoveredWinsServers)
        {
            Dictionary<int, WindowsServer> newDnsList = new Dictionary<int, WindowsServer>();
            int order = 1;

            foreach (var dc in discoveredWinsServers.OrderBy(x => x.Order))
            {
        ...
        private void LogDiscoveredServer(Dictionary<int, WindowsServer> discoveredWinsServers)
        {
            foreach (var winsServer in discoveredWinsServers)
            {
                Logger.Debug(string.Format("Discoverd WINS{0} server {3}:{1} from site {2}", winsServer.Key, winsServer.Value.IPAddress, winsServer.Value.SiteName, winsServer.Value.Name));
            }
        }
